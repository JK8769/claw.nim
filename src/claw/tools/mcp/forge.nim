...
""".format(importBlock, name, bodyCode.indent(2))
    ...
    if "mcpServer" notin code:
      return "Error: Missing 'mcpServer' macro. You MUST provide a full Nim script OR set 'logic_only: true'. See the `tools` skill (forge section) for the required boilerplate."
