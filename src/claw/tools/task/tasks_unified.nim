## Unified nimclaw platform tool — task board operations.
## Single tool with `action` parameter dispatching to assign / claim / submit.
## (Inter-agent mail is in `tools/mail_unified.nim`.)
...
method name*(t: NimclawTool): string = "task"

method description*(t: NimclawTool): string =
  "NimClaw platform task-board operations.\n\n" &
  "Actions:\n" &
  ...
  "For inter-agent mail use the `mail` tool."
...
method execute*(t: NimclawTool, args: Table[string, JsonNode]): Future[string] {.async.} =
  ...
  of "submit":
    ...
    return "Error: Unknown action '" & action & "'. Use: assign, claim, submit"
