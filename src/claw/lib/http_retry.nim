...
# Use the vendored curl shim with cancel API support — keeps Curly
# type consistent across the http stack so the cancel-in-flight call
# site in providers/http.nim works on the same object instance.
import curl as curly, webby/httpheaders
import ../logger
