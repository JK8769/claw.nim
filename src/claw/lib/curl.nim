...
import std/strutils, std/locks, std/random, webby/httpheaders, zippy,
    std/deques, std/tables, std/options, std/times, std/os
...
type
  ...
  CurlyObj* = object
    ...
    thread: Thread[Curly]
    closeCalled: bool
    cancelAllPending: bool   # set by cancelAllInFlight*, picked up by worker
                             # on its next loop iteration. Causes every
                             # in-flight request to be removed from libcurl
                             # via multi_remove_handle and signaled with
                             # an error, unblocking any caller waiting on
                             # the request's waitGroup or streamState.
...
proc threadProc(curl: Curly) {.raises: [].} =
  ...
  while true:
    ...
    # cancelAllInFlight* support: when the flag is set (from another
    # thread, e.g. the agent loop's /session stop handler), abort
    # every in-flight request and unblock its waitGroup/streamState.
    # Signal handling mirrors the timeout path above PLUS the proper
    # waitGroup.done() / requestsCompleted append from line 405-410,
    # so callers blocked in either streaming or non-streaming mode
    # are released.
    let cancelAll = block:
      var v: bool
      withLock curl.lock:
        v = curl.cancelAllPending
        curl.cancelAllPending = false
      v
    if cancelAll:
      var toCancel: seq[RequestWrap] = @[]
      for _, request in curl.inFlight:
        toCancel.add(request)
      for request in toCancel:
        discard multi_remove_handle(curl.multiHandle, request.easyHandle)
        curl.inFlight.del(request.easyHandle)
        request.error = "Cancelled by user request " & request.verb & ' ' & request.url
        easy_reset(request.easyHandle)
        curl.availableEasyHandles.addLast(request.easyHandle)
        if request.streamState.isSome:
          withLock request.streamState.get.lock:
...
proc cancelAllInFlight*(curl: Curly) {.gcsafe.} =
  ## Cancel all currently in-flight HTTP requests. Sets a flag the
  ## worker thread picks up on its next iteration (typically within
  ## ~2-200ms since multi_perform polls with a 2ms timeout). Each
  ## affected request is removed from libcurl via multi_remove_handle,
  ## its `error` field is set to "Cancelled by user request ...",
  ## and its waitGroup / streamState is signaled — so blocking
  ## callers (sync `request*`/`get*`/`post*`) unblock immediately
  ## with an error response.
  ##
  ## Used by claw's /session stop slash command to interrupt hung
  ## LLM HTTP calls without waiting for libcurl's internal timeout
  ## (which can be minutes when the server keeps the connection
  ## alive without sending body data).
  withLock curl.lock:
    curl.cancelAllPending = true
  signal(curl.cond)   # wake the worker if it's sleeping on no-work
...
# ── Claw additions ─────────────────────────────────────────────────
# Lives at the bottom because it depends on `post*` defined above.

proc postWithRetry*(curl: Curly, url, body: string, headers: HttpHeaders,
                    timeout: int = 30, maxRetries: int = 9):
                    tuple[code: int, body: string] =
  ## POST with exponential backoff on transient transport-layer errors.
  ## Default 9 retries with 1/2/4/8/16/32/64/128s exponential backoff
  ## (+jitter) — total worst-case ~4.25min. Tuned for CN→US transit,
  ## where flaps can last tens of seconds; the long tail catches
  ## sustained outages without prematurely consuming a fallback-chain
  ## entry.
  randomize()
  for attempt in 1..maxRetries:
    try:
      let resp = curl.post(url, headers = headers, body = body, timeout = timeout)
      return (resp.code, resp.body)
    except Exception as e:
      let msg = e.msg
      # Transport-layer failures we want to retry. `peer` /
      # `receiving data` catches CURLE_RECV_ERROR (the dominant
      # CN→US transit hiccup); `Recv failure`, `transfer closed`,
      # `HTTP/2`, `EOF` cover the other variants curl reports for
      # the same class of problem. SSL / timeout / connection /
      # reset / refused / resolve are the original retryable set.
      let isRetryable =
      ...
      if attempt < maxRetries and isRetryable:
        ...
        warnCF("curl", "Request failed, retrying", {
          "attempt": $attempt,
          "delay_ms": $sleepDelay,
          "error": msg
        }.toTable)
