...
name: alarm-response
version: 1.0.0
description: "Classify active plant alarms by severity, group by plant for operator efficiency, escalate critical ones immediately. Workflow over fleet_inverter_alarms + delegate."
loading: lazy
operations:
  - scan
  - classify
...
- **Reactive**: when a `fleet_plant_now` call returns
  `alarms_count > 0` and the agent decides to investigate.

## Workflow

1. Call `fleet_plant_list()` — get all plants with their current
   `alarms_count`. Filter to plants where count > 0.
2. For each plant with active alarms, call
   `fleet_inverter_alarms(plant_id)` to get alarm details.
