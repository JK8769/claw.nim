...
version: 1.0.0
description: "Verify and invite a new customer for solar plant monitoring: check Known Entities first; if not present, mint via `social method=invite`; quote the tool result verbatim. Anti-fabrication discipline: no nc: ids asserted from memory."
loading: lazy
operations:
  - verify
  - invite
...
The customer's own redeem flow uses `social method=redeem` directly,
...
4. **If Known Entities doesn't match**, optionally call
   `social method=query` for a second-source check (search by
   display name in case the identifier resolution was incomplete).

5. **Call `social method=invite`** with the resolved identifier:
...
- Call `social method=query` to look up the name
- Call `social method=invite` and let it fail-loudly if the
