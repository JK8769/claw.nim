...
back-and-forth that should have been a single `fleet_plant_list()`.
...
…without calling `fleet_plant_list`, `delegate`, or another tool
...
```
fleet_plant_list()             → all plants, capacities, statuses
fleet_plant_now(plant_id=X)    → current power + today's yield
```

For a "how are my plants doing" question, `fleet_plant_list()`
...
follow up with `fleet_plant_now(plant_id=…)`.
...
```
fleet_plant_list()                                    → pick plant_id(s)
fleet_plant_history(plant_id=X, from=date, to=date)   → daily yield rows
```
...
|---|---|---|
| **API returned empty** | Upstream server didn't return rows | `fleet_plant_history` returns `[]` or all-`missing` |
