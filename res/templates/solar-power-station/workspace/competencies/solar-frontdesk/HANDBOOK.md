...
back-and-forth that should have been a single `fleet action=plant_list()`.
...
…without calling `fleet action=plant_list`, `delegate`, or another tool
...
```
fleet action=plant_list()             → all plants, capacities, statuses
fleet action=plant_now(plant_id=X)    → current power + today's yield
```

For a "how are my plants doing" question, `fleet action=plant_list()`
...
follow up with `fleet action=plant_now(plant_id=…)`.
...
```
fleet action=plant_list()                                    → pick plant_id(s)
fleet action=plant_history(plant_id=X, from=date, to=date)   → daily yield rows
```
...
|---|---|---|
| **API returned empty** | Upstream server didn't return rows | `fleet action=plant_history` returns `[]` or all-`missing` |
